...
bool CS2ABanManager::AddBan(const char *authid, int time, const char *reason, int adminSlot)
{
...
bool CS2ABanManager::BanIP(const char *ip, int time, const char *reason, int adminSlot)
{
...
bool CS2ABanManager::Unban(const char *authid, int adminSlot)
{
	...
	snprintf(query, sizeof(query),
			 ...
			 "AND (length = '0' OR ends > %lld) "
			 "AND RemoveType IS NULL",
			 prefix.c_str(), prefix.c_str(), adminAuth.c_str(), adminMatch.c_str(), now, targetMatch.c_str(), now);
...
bool CS2ABanManager::UnbanIP(const char *ip, int adminSlot)
{
	...
	snprintf(query, sizeof(query),
			 ...
			 "AND (length = '0' OR ends > %lld) "
			 "AND RemoveType IS NULL",
			 prefix.c_str(), prefix.c_str(), adminAuth.c_str(), adminMatch.c_str(), now, escapedIP.c_str(), now);
								  ...
								  std::function<void(int banCount, int commCount, int muteCount, int gagCount)> callback)
{
	...
	g_CS2ADatabase.Query(query,
						 [callback](ISQLQuery *result)
						 {
							 ...
							 if (callback)
							 {
								 callback(banCount, muteCount + gagCount, muteCount, gagCount);
							 }
						 });
}
...
void CS2ABanManager::CheckSleuth(int slot, uint64_t steamid64, const char *ip)
{
	...
	{
		char timeBuf[128];
		snprintf(timeBuf, sizeof(timeBuf), " AND created > %lld", now - (long long)g_CS2AConfig.sleuthExcludeTime);
		timeFilter = timeBuf;
	}
	...
	g_CS2ADatabase.Query(
		...
		[slot, steamid64](ISQLQuery *result)
		{
			...
			// A length of 0 means permanent, so one such row outranks any finite MAX(length).
			if (rs->GetInt(2) == 0)
			{
				originalLength = 0;
			}
...
// The caller's slot can change hands while a list query is in flight, and the output would then go to whoever took it.
// The server console (slot -1) always prints.
static bool CallerStillThere(int callerSlot, uint64_t callerSteamid64)
{
	if (callerSlot < 0)
	{
		return true;
...
{
	PlayerInfo *caller = g_CS2APlayerManager.GetPlayer(callerSlot);
	return caller ? caller->steamid64 : 0;
}
